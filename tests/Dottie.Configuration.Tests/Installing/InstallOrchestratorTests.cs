...
public class InstallOrchestratorTests
{
    ...
    public async Task InstallAsync_CallsAllRegisteredInstallersAsync()
    {
        ...
        _mockInstaller
            .Setup(x => x.InstallAsync(installBlock, context, It.IsAny<Action<InstallResult>?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new[]
            {
        ...
        // Assert
        _mockInstaller.Verify(x => x.InstallAsync(installBlock, context, It.IsAny<Action<InstallResult>?>(), It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task InstallAsync_WithDryRun_PassesContextToInstallersAsync()
    {
        ...
        _mockInstaller
            .Setup(x => x.InstallAsync(installBlock, It.Is<InstallContext>(c => c.DryRun), It.IsAny<Action<InstallResult>?>(), It.IsAny<CancellationToken>()))
        ...
        _mockInstaller.Verify(
            x => x.InstallAsync(installBlock, It.Is<InstallContext>(c => c.DryRun), It.IsAny<Action<InstallResult>?>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }
